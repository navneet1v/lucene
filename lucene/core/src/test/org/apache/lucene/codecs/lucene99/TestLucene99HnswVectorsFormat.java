    ...
    try (Directory dir = newDirectory();
      ...
      try (IndexReader reader = DirectoryReader.open(w)) {
        ...
        if (r instanceof CodecReader codecReader) {
          ...
          // vex may or may not be present depending on whether HNSW graph was built
          // (tiny segments may skip graph building)
          if (offHeap.containsKey("vex")) {
            assertTrue(offHeap.get("vex") >= 0);
          }
          assertTrue(offHeap.size() >= 1 && offHeap.size() <= 2);
        }
      }
    }
  }
