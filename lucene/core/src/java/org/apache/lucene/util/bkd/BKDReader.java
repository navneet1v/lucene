...
public class BKDReader extends PointValues {
  ...
  private static class BKDPointTree implements PointTree {
    ...
    public void prefetchAll(TwoPhaseIntersectVisitor visitor, boolean grown) throws IOException {
      ...
      if (isLeafNode()) {
        ...
        if (visitor.prefetchMode() == TwoPhaseIntersectVisitor.PrefetchMode.FIRST_MATCH) {
  ...
  public interface TwoPhaseIntersectVisitor extends IntersectVisitor {
    ...
    /** Returns the prefetch mode for this visitor. */
  ...
  public abstract static class BaseTwoPhaseIntersectVisitor implements TwoPhaseIntersectVisitor {
    ...
    private static volatile PrefetchMode globalPrefetchMode = PrefetchMode.FIRST_MATCH;

    public static void setGlobalPrefetchMode(PrefetchMode mode) {
      globalPrefetchMode = mode;
    }

    public static PrefetchMode getGlobalPrefetchMode() {
      return globalPrefetchMode;
    }

    int lastDeferredBlockOrdinal = -1;
    List<Long> deferredBlocks = new ArrayList<>();
    private final PrefetchMode prefetchMode;

    protected BaseTwoPhaseIntersectVisitor() {
      this(PrefetchMode.FIRST_MATCH);
    }

    protected BaseTwoPhaseIntersectVisitor(PrefetchMode prefetchMode) {
      this.prefetchMode = prefetchMode;
    }

    @Override
    public PrefetchMode prefetchMode() {
      return prefetchMode;
    }
